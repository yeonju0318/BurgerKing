...
function Store() {
  ...
  useEffect(() => {
    ...
    const my_script = new_script(
      "https://dapi.kakao.com/v2/maps/sdk.js?autoload=false&appkey=595018d6bab7f70a6edd2a3ff77d999c"
    );
        ...
        <StSearchBar>
          ...
          <StSearch>
            <div onClick={siDropdown}>
              <StCategory>{selected_Si}</StCategory>
              {siOpen &&
                search_Si.map((item) => {
                  return (
                    <div key={item} onClick={() => si_ClickHandler(item)}>
                      {item}
                    </div>
                  );
                })}
            ...
            <div onClick={guDropdown}>
              <StCategory>{selected_Gu}</StCategory>
              {guOpen &&
                search_Gu.map((item) => {
                  return (
                    <div key={item} onClick={() => gu_ClickHandler(item)}>
                      {item}
                    </div>
                  );
                })}
